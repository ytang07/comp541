...
module datapath#(
   ...
   parameter Dbits = 32                         // Number of bits in data
    ...
    assign newPC = (pcsel == 2'b11) ? JT
                        ...
                        : (pcsel == 2'b01) ? BT
                        : pcPlus4;
    programcounter pcm(.clk(clk), .reset(reset), .enable(enable), .D(newPC), .pc(pc));
    assign pcPlus4 = pc + 4;
    ...
    register_file #(Nloc, Dbits) rf(.clk(clk), .wr(werf), .ReadAddr1(instr[25:21]), .ReadAddr2(instr[20:16]),
